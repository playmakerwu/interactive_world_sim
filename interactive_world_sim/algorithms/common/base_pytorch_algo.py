...
class BasePytorchAlgo(pl.LightningModule, ABC):
    ...
    See https://lightning.ai/docs/pytorch/stable/starter/introduction.html for more.
    """
    ...
    def __init__(self, cfg: DictConfig):
        ...
        dtype = torch.float32 if "dtype" not in self.cfg else self.cfg.dtype
        self = self.to(dtype)
        self._build_model()
    ...
    def training_step(self, *args: Any, **kwargs: Any) -> STEP_OUTPUT:
        r"""Compute training loss for a single batch.

        Args:
            batch: The output of your data iterable, normally a
                :class:`~torch.utils.data.DataLoader`.
            batch_idx: The index of this batch.
            dataloader_idx: (only if multiple dataloaders used) The index of the
                dataloader that produced this batch.
        ...
        Return:
            ...
            - ``dict`` - A dictionary. Can include any keys, but must include the key
              ``'loss'``.
            - ``None`` - Skip to the next batch. This is only supported for automatic
                optimization. This is not supported for multi-GPU, TPU, IPU,
                or DeepSpeed.
    ...
    def configure_optimizers(self) -> torch.optim.Optimizer:
        ...
        If you need to use more than one optimizer, refer to pytorch lightning:
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            mean: optional, the mean to unnormalize image tensor, assuming unnormalized
                data is in [0, 1].
            std: optional, the std to unnormalize tensor, assuming unnormalized data is
                in [0, 1].
    ...
    def log_gradient_stats(self) -> None:
        ...
        with torch.no_grad():
            ...
            gpr_tensor = torch.tensor(gpr)
            self.log_dict(
                {
                    "train/grad_norm/min": grad_norms_tensor.min(),
                    "train/grad_norm/max": grad_norms_tensor.max(),
                    "train/grad_norm/std": grad_norms_tensor.std(),
                    "train/grad_norm/mean": grad_norms_tensor.mean(),
                    "train/grad_norm/median": torch.median(grad_norms_tensor),
                    "train/gpr/min": gpr_tensor.min(),
                    "train/gpr/max": gpr_tensor.max(),
                    "train/gpr/std": gpr_tensor.std(),
                    "train/gpr/mean": gpr_tensor.mean(),
                    "train/gpr/median": torch.median(gpr_tensor),
                }
            )
